...
function T(N,F){
	...
	for(var K in $)
		...
		if(F){
			...
			N.eCont=$dp.$(N.eCont);
			N.el=S;N.autoPickDate=true;
			N.qsEnabled=false;I(N)
		}else{
			...
			if(!N.el||N.el["My97Mark"]===true||N.el.disabled||($dp.dd&&O($dp.dd)!="none"&&$dp.dd.style.left!="-970px")){
				try{if(N.el["My97Mark"])N.el["My97Mark"]=false}
				catch(C){}return
			}
			if(J&&N.el.nodeType==1&&N.el["My97Mark"]===undefined){
				var A,D;
				if(J.type=="focus")E(N.el,"onclick",function(){T(N)});
...
function E(I,H){
	var E_A;
	if(H!=undefined && H.$dpPath!=undefined)
		E_A=H.$dpPath;
	else
		E_A=A;
	...
	for(var L=0;L<C.length;L++){
		if(C[L].name==H.skin){
			J.push("<link rel=\"stylesheet\" type=\"text/css\" href=\""+E_A+"skin/"+C[L].name+"/datepicker.css\" charset=\""+C[L].charset+"\"/>");
			J.push("<script type=\"text/javascript\" src=\""+E_A+"calendar.js?\"+Math.random()+\"\"></script>");
			...
			H.setPos=B;H.onload=Y;F.write("<html>");
			F.cfg=H;F.write(J.join(""));F.close()
		}
	}
}
...
function B(J){
	...
	if(($.topM+I.bottom+G>E.height)&&($.topM+I.top-G>0))C+=B.top+$.topM+I.top-G-2;
		...
		C+=B.top+$.topM+I.bottom;var _=C-B.top+G-E.height;
		if(_>0)C-=_
	}if(isNaN(H))H=0;
